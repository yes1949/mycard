...
<head>
    ...
    <style>
        ...
        /* ----------------------- 新增筛选框样式 ----------------------- */
        .filter-section {
            width: 50%; /* 调整宽度以容纳更多按钮 */
            margin: 1.5vw auto 0.5vw auto; /* 调整上边距，使其靠近统计卡片，下方留出一点空间 */
            ...
            gap: 2.5vw; /* 按钮间距更大一些，防止太挤 */
            box-sizing: border-box; /* 包含padding在内的宽度计算 */
        }

        .filter-dot {
            width: 7vw; /* 增大一点点，使圆形更明显 */
            height: 7vw;
            line-height: 7vw; /* 垂直居中文字 */
            border-radius: 50%;
            ...
            user-select: none;
            /* 重新计算flex-basis，以适应四个按钮和间距 */
            flex: 0 0 calc((100% - 3 * 2.5vw) / 4); /* (100% - 3个gap) / 4个按钮 */
        ...
        .progress-display {
            ...
            color: #666;
            text-align: center;
            margin-top: 0.5vw; /* 适当减小上边距，使其更靠近圆点按钮 */
            min-height: 20px; /* 预留空间避免跳动 */
            user-select: text;
        }
    </style>
</head>
<body>
    ...
    <div id="page1" class="page-container active">
        <div class="stats-card" id="statsCard">
            <div id="statsHint" class="stats-hint">📊 </div>
        ...
        const app = {
            ...
            page1: {
                data: { 
                    ...
                    // 模式0 (allword) 进度
                    i: 0, // 当前抽卡片次
                    j: 0, // allword1的元素数量 (初始allword的数量)
                    k: 0, // 轮次
                    allword: [], // 所有卡片
                    allword1: [], // allword的副本，用于抽卡时删除
                    
                    // 模式1 (todayword) 进度
                    x: 0, // 当前抽卡片次
                    n: 0, // todayword1的元素数量 (初始todayword的数量)
                    m: 0, // 轮次
                    todayword: [], // pastday符合条件的卡片
                    todayword1: [], // todayword的副本，用于抽卡时删除

                    // 模式-1 (todayNewWord) 进度
                    y: 0, // 当前抽卡片次
                    z: 0, // todayNewWord1的元素数量 (初始todayNewWord的数量)
                    p: 0, // 轮次
                    todayNewWord: [], // todayNewWord卡片
                    todayNewWord1: [], // todayNewWord的副本，用于抽卡时删除
                },
                elements: {
                    ...
                    progressDisplay: document.getElementById('progressDisplay'), // 新增进度显示元素
                    // 新增统计卡片相关元素
                    statsCard: document.getElementById('statsCard'),
                    statsContent: document.getElementById('statsContent'),
                    statsHint: document.getElementById('statsHint')
                },
                filterOptions: ['A', '0', '1', '-1'], // 新的筛选按钮，增加-1
                // 定义 todayword 筛选条件
                TODAY_PASTDAYS_FILTER: [0, 1, 4, 7, 15, 30, 60, 90, 120],

                onLoad() { 
                    this.addEventListeners(); 
                    this.handleDailyDataInitialization(); // 每日数据初始化
                ...
                onShow() {
                    ...
                    this.getRandomContent(false); // 刷新显示当前模式下的卡片
                    this.updateProgressDisplay(); // 更新进度显示
                },
                addEventListeners() {
                    ...
                    // 新增：统计卡片点击切换逻辑
                    this.elements.statsCard.addEventListener('click', () => {
                        if (this.elements.statsContent.style.display === 'none') {
                            this.elements.statsContent.style.display = 'block';
                            this.elements.statsHint.style.display = 'none'; // 展开时隐藏提示
                        } else {
                            this.elements.statsContent.style.display = 'none';
                            this.elements.statsHint.style.display = 'block'; // 收起时显示提示
                        }
                    });
                },
                renderFilterDots() {
                ...
                changeMode(newMode) {
                    if (this.data.currentMode === newMode) return; // 如果是当前模式，不做任何操作

                    // 在切换模式前保存当前模式的进度
                    this.saveProgress();

                    this.data.currentMode = newMode;
                    this.renderFilterDots(); // 更新按钮选中状态
                    this.getRandomContent(false); // 切换模式后，显示新的内容
                    this.updateProgressDisplay(); // 更新进度显示
                },
                
                // 每日数据初始化（包括allword, todayword, todayNewWord和进度变量）
                handleDailyDataInitialization() {
                    ...
                    if (lastInitDate !== today) {
                        ...
                        wx.setStorageSync('lastInitDate', today); // 记录本次更新日期

                        // 2. 清空并重新构建 allword, todayword, todayNewWord (每次新的一天都重新从 myCards 构建)
                        this.data.allword = [...cards];
                        this.data.todayword = cards.filter(card => 
                            this.TODAY_PASTDAYS_FILTER.includes(card.pastday)
                        );
                        this.data.todayNewWord = cards.filter(card => card.pastday === 0); // 新增：过滤 pastday 为 0 的卡片

                        wx.setStorageSync('allword', this.data.allword);
                        wx.setStorageSync('todayword', this.data.todayword);
                        wx.setStorageSync('todayNewWord', this.data.todayNewWord); // 保存今日新增卡片

                        // 3. 重置所有进度变量并清空副本 (新的一天重置所有进度)
                        this.data.i = 0;
                        this.data.j = 0;
                        this.data.k = 0;
                        this.data.x = 0;
                        this.data.n = 0;
                        this.data.m = 0;
                        this.data.y = 0; // 新增：重置今日新增卡片进度
                        this.data.z = 0; // 新增：重置今日新增卡片总数
                        this.data.p = 0; // 新增：重置今日新增卡片轮次

                        this.data.allword1 = []; // 清空副本，等待第一次抽取时重新加载
                        this.data.todayword1 = []; // 清空副本，等待第一次抽取时重新加载
                        this.data.todayNewWord1 = []; // 新增：清空副本，等待第一次抽取时重新加载

                        this.saveProgress(); // 保存重置后的进度
                        
                        // 重置当日刷卡计数
                        wx.setStorageSync('todaySwipeData', { date: today, count: 0 });
                    } else {
                        // 如果是同一天，加载已有的 allword, todayword, todayNewWord
                        // 注意：如果 myCards 在当天有变化，allword, todayword, todayNewWord 需要从 myCards 重新生成，而不是直接从 localStorage 读取
                        this.data.allword = [...cards];
                        this.data.todayword = cards.filter(card => 
                            this.TODAY_PASTDAYS_FILTER.includes(card.pastday)
                        );
                        ...
                        this.data.todayNewWord1 = wx.getStorageSync('todayNewWord1') || []; // 新增：加载今日新增卡片副本
                    }
                ...
                loadProgress() {
                    ...
                    // 只有当日期匹配时才加载进度，否则由 handleDailyDataInitialization 重置
                    if (lastProgressDate === today) {
                        ...
                        this.data.y = savedProgress.y || 0; // 新增
                        this.data.z = savedProgress.z || 0; // 新增
                        this.data.p = savedProgress.p || 0; // 新增
                        this.data.currentMode = savedProgress.currentMode || 'A';
                    } else {
                        // 如果日期不匹配，handleDailyDataInitialization 已经重置了，这里确保模式是默认A
                        this.data.currentMode = 'A';
                    }
                    this.renderFilterDots(); // 确保加载进度后，按钮状态正确
                },
                ...
                saveProgress() {
                    ...
                    wx.setStorageSync('todayNewWord1', this.data.todayNewWord1); // 新增
                },
                ...
                updateProgressDisplay() {
                    ...
                    if (this.data.currentMode === '0') {
                        ...
                        if (this.data.j > 0 || (this.data.allword.length > 0 && this.data.k === 0 && this.data.i === 0)) {
                             // 如果是第一轮还没开始抽卡，显示allword的实际总数
                            let currentJ = this.data.j > 0 ? this.data.j : this.data.allword.length;
                            displayString = `${currentJ - this.data.allword1.length}-${currentJ}-${this.data.k}`; // 修正i的计算
                        }
                    } else if (this.data.currentMode === '1') {
                        // 当n被初始化后才显示
                        if (this.data.n > 0 || (this.data.todayword.length > 0 && this.data.m === 0 && this.data.x === 0)) {
                            // 如果是第一轮还没开始抽卡，显示todayword的实际总数
                            let currentN = this.data.n > 0 ? this.data.n : this.data.todayword.length;
                            displayString = `${currentN - this.data.todayword1.length}-${currentN}-${this.data.m}`; // 修正x的计算
                        }
                    } else if (this.data.currentMode === '-1') { // 新增 -1 模式的进度显示
                        if (this.data.z > 0 || (this.data.todayNewWord.length > 0 && this.data.p === 0 && this.data.y === 0)) {
                            ...
                            displayString = `${currentZ - this.data.todayNewWord1.length}-${currentZ}-${this.data.p}`; // 修正y的计算
                        }
                    }
                ...
                getRandomContent(isUserAction = false) { 
                    ...
                    let currentCardsPool = []; // 当前模式下用于抽卡的卡片池
                    let selectedCard = null;
                    let selectedCardIndexInPool = -1; // 在当前卡片池中的索引

                    if (this.data.currentMode === 'A') {
                        currentCardsPool = [...this.data.allword]; // 始终从 allword 的最新副本中抽取
                        ...
                        if (this.data.allword1.length === 0) {
                            ...
                            // 如果 allword 本身为空，allword1 也为空，直接返回
                            if (this.data.allword1.length === 0) {
                                ...
                                this.saveProgress(); // 保存状态
                                return;
                            }
                            this.data.j = this.data.allword.length; // 记录本轮总数
                            this.data.i = 0; // 重置本轮计数
                            this.data.k++; // 轮次加1
                        }
                        currentCardsPool = this.data.allword1;
                        selectedCardIndexInPool = Math.floor(Math.random() * currentCardsPool.length);
                        selectedCard = currentCardsPool[selectedCardIndexInPool];
                        if (isUserAction && selectedCard) { // 只有用户点击时才删除并更新计数
                            this.data.allword1.splice(selectedCardIndexInPool, 1);
                            this.data.i = this.data.j - this.data.allword1.length; // 修正i的计算
                        }
                        ...
                        if (this.data.todayword1.length === 0) {
                            ...
                            // 如果 todayword 本身为空，todayword1 也为空，直接返回
                            if (this.data.todayword1.length === 0) {
                                this.elements.displayContent.textContent = '暂无内容，请添加';
                                this.data.currentIndex = -1;
                                this.updateStatistics();
                            ...
                            this.data.n = this.data.todayword.length; // 记录本轮总数
                            this.data.x = 0; // 重置本轮计数
                            this.data.m++; // 轮次加1
                        }
                        ...
                        if (isUserAction && selectedCard) { // 只有用户点击时才删除并更新计数
                            ...
                            this.data.x = this.data.n - this.data.todayword1.length; // 修正x的计算
                        }
                    } else if (this.data.currentMode === '-1') { // 新增 -1 模式逻辑
                        ...
                        selectedCardIndexInPool = Math.floor(Math.random() * currentCardsPool.length);
                        selectedCard = currentCardsPool[selectedCardIndexInPool];
                        if (isUserAction && selectedCard) {
                            this.data.todayNewWord1.splice(selectedCardIndexInPool, 1);
                            this.data.y = this.data.z - this.data.todayNewWord1.length; // 修正y的计算
                        }
                    }
                ...
                confirmDelete() {
                    ...
                    if (indexToDelete >= 0 && indexToDelete < myCards.length) { 
                        ...
                        // 2. 重新构建 allword, todayword, todayNewWord (确保是最新的，已剔除删除的卡片)
                        this.data.allword = [...myCards];
                        this.data.todayword = myCards.filter(card => 
                            this.TODAY_PASTDAYS_FILTER.includes(card.pastday)
                        );
                        this.data.todayNewWord = myCards.filter(card => card.pastday === 0); // 新增：重新构建今日新增卡片

                        wx.setStorageSync('allword', this.data.allword);
                        wx.setStorageSync('todayword', this.data.todayword);
                        wx.setStorageSync('todayNewWord', this.data.todayNewWord); // 保存今日新增卡片

                        // 3. 更新所有模式下的副本和进度值 J, N, Z
                        // 从 allword1 中删除（如果存在）
                        ...
                        // 从 todayword1 中删除（如果存在）
                        this.data.todayword1 = this.data.todayword1.filter(card => 
                            !(card.word === deletedCard.word && card.pastday === deletedCard.pastday && card.createDate === deletedCard.createDate)
                        );
                        // 从 todayNewWord1 中删除（如果存在）
                        this.data.todayNewWord1 = this.data.todayNewWord1.filter(card => 
                            !(card.word === deletedCard.word && card.pastday === deletedCard.pastday && card.createDate === deletedCard.createDate)
                        );
                        
                        // 更新 J, N, Z 值 (当前轮次的总量)
                        this.data.j = this.data.allword.length; // 重新设定 J 为新的 allword 总数
                        this.data.n = this.data.todayword.length; // 重新设定 N 为新的 todayword 总数
                        this.data.z = this.data.todayNewWord.length; // 新增：重新设定 Z 为新的 todayNewWord 总数

                        // 如果当前模式的副本因删除而清空，则重置本轮计数
                        if (this.data.currentMode === '0') {
                            this.data.i = this.data.j - this.data.allword1.length;
                            if (this.data.allword1.length === 0 && this.data.j > 0) { // 如果删除后为空且原总数不为0，意味着该轮次已完成
                                this.data.k++; // 轮次加1
                                this.data.allword1 = [...this.data.allword]; // 重新加载副本开启新轮次
                                this.data.i = 0; // 重置本轮已抽次数
                            } else if (this.data.allword1.length === 0 && this.data.j === 0) { // 如果删除后为空且原总数也为0
                                this.data.i = 0;
                            }
                        }
                        ...
                        if (this.data.currentMode === '-1') { // 新增：更新-1模式进度
                        ...
                        this.saveProgress(); // 保存更新后的所有进度数据和副本
                        ...
                        this.getRandomContent(false); // 刷新显示下一张卡片
                    }
                    this.cancelDelete();
                },
            ...
            page2: {
                ...
                saveCard() {
                    ...
                    if (uniqueNewCards.length > 0) {
                        ...
                        // 更新 allword, todayword, todayNewWord
                        ...
                        app.page1.data.todayNewWord = updatedCards.filter(card => card.pastday === 0); // 新增：更新今日新增卡片

                        wx.setStorageSync('allword', app.page1.data.allword);
                        wx.setStorageSync('todayword', app.page1.data.todayword);
                        wx.setStorageSync('todayNewWord', app.page1.data.todayNewWord); // 保存今日新增卡片
                        ...
                        app.page1.data.z = app.page1.data.todayNewWord.length; // 新增：更新 Z
                ...
                copyAllContents() {
                    ...
                    if (currentMode === 'A') {
                        ...
                        cardsToCopy = app.page1.data.allword; // 复制全部卡片模式的总卡片池
                    } else if (currentMode === '1') {
                        cardsToCopy = app.page1.data.todayword; // 复制今日复习模式的总卡片池
                    } else if (currentMode === '-1') {
                        cardsToCopy = app.page1.data.todayNewWord; // 复制今日新增模式的总卡片池
                    }
                    ...
                    wx.showModal({
                        ...
                        success: (res) => { 
                            if (res.confirm) { 
                                ...
                                wx.removeStorageSync('todayNewWord'); // 新增：清空今日新增卡片
                                wx.removeStorageSync('allword1'); 
                                wx.removeStorageSync('todayword1'); 
                                wx.removeStorageSync('todayNewWord1'); // 新增：清空今日新增卡片副本
                                wx.removeStorageSync('lastInitDate'); 

                                // 重置 page1 的所有相关数据和状态
                                app.page1.data.allword = [];
                                app.page1.data.allword1 = [];
                                app.page1.data.todayword = [];
                                app.page1.data.todayword1 = [];
                                app.page1.data.todayNewWord = []; // 新增
                                app.page1.data.todayNewWord1 = []; // 新增
                                ...
                                app.page1.data.n = 0;
                                app.page1.data.m = 0;
                                app.page1.data.y = 0; // 新增
                                app.page1.data.z = 0; // 新增
                                app.page1.data.p = 0; // 新增
